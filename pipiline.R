...
data.cont<-data %>% group_by(count,activity_day) %>% tally()
movt.larg<-data.cont %>% filter(n>=10) %>%  print()

#  Data winsoring
require(DescTools)
win.data<-data  %>% mutate(win.var = Winsorize(data$count, minval = tail(Small(data$count, k = 3),1), maxval = head(Large(data$count, k = 3),1))) %>% print() # Here I do not know the reasonable range for a winsorization
